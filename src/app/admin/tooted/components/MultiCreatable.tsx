...
import { default as Select, components, InputAction } from "react-select";
import Creatable, { useCreatable } from "react-select/creatable";
export type Option = {
  value: number | string;
  label: string;
};
//https://codesandbox.io/p/sandbox/react-multi-select-example-with-select-all-option-and-checkboxes-ejjc9?file=%2Fsrc%2FApp.tsx%3A20%2C1
const MultiSelectable = forwardRef((props: any, ref) => {
  const [selectInput, setSelectInput] = useState<string>("");
  const isAllSelected = useRef<boolean>(false);
  const selectAllLabel = useRef<string>("Vali kõik");
  ...
  if (props.isSelectAll && props.options.length !== 0) {
    ...
    if (filteredSelectedOptions?.length > 0) {
      if (filteredSelectedOptions?.length === filteredOptions?.length)
        selectAllLabel.current = `Kõik (${filteredOptions.length}) valitud`;
      else
        selectAllLabel.current = `${filteredSelectedOptions?.length} / ${filteredOptions.length} valitud`;
    } else selectAllLabel.current = "Vali kõik";
